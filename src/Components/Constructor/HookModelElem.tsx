...
const Node: React.FC<{ node: IHOOK_Node } & HTMLAttributes<HTMLDivElement>> = ({ node }): JSX.Element => {
    return (
        <div className={`flex-col h-[10em] min-w-[3em] border-8 border-double border-black bg-[#0f66ad] justify-items-start`} >
            ...
            <div>{node.id}</div>
            <div className='text-white'>row: {node.row_lvl}</div>
        </div>)
}
...
const HookModelElem = (props: Props) => {
    ...
    const add = (model_id: string, lvl = 0, row_id?: string) =>
        setModels(prev => prev.map(mod => mod.id === model_id ? { ...mod, nodes: [...mod.nodes, new HookNode(lvl, row_id)] } : mod))
    ...
    const rem = (model_id: string, lvl: number, row_id: string) => {
        ...
        const lvlN = list_rows.filter(n => n.row_lvl === lvl)
                ...
                list_rows && list_rows.map((list_item) =>
                    <div key={list_item.id} className='relative'>
                        {list_item.row_nodes.length > 1 && <button className='absolute right-[-3em] bottom-2 border-2 bg-[#450563] p-1 mt-1 rounded-md border-[black]'
                            onClick={() => { rem(model.id, list_item.row_lvl, list_item.row_id) }}
                        >
                            <IcMinus hw={6} />
                        </button>}
                        {list_item.row_nodes.length < 5 &&
                            <button className='absolute left-[-3em] bottom-2 border-2 bg-[#450563] p-1 mt-1 rounded-md border-[black]'
                                onClick={() => { add(model.id, list_item.row_lvl, list_item.row_id) }}
                            >
                                <IcPlus hw={6} />
                            </button>
