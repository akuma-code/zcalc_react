...
export function useNodeBorders(borders?: INodeBorder[]) {
    ...
    const applyDelta = (border: INodeBorder & { state: ISideStateValues }): BD => ({ ...border, delta: delta[border.state]!, desc: BorderDescEnum[border.state as keyof typeof BorderDescEnum] })
